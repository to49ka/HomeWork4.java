...
public class HomeWork4 {
     ...
     * У Вас есть список stringList, элементами которюого являются строки.
    ...
    public static List<String> correctedList(List<String> stringList) {
        ...
        for (int i = 0; i < stringList.size(); i++) {
            boolean error = false;
            for (int a = 0; a < stringList.get(i).length(); a++) {
                if (stringList.get(i).charAt(a) >= 'A' && (stringList.get(i).charAt(a) <= 'Z')) {
                    error = true;
                }
            }
            if (!error) {
    ...
    public static HashMap<String, Integer> magSort(HashMap<String, Integer> shopCart) {
        ...
        for (String b : shopCart.keySet()) {
            if (shopCart.get(b) <= 300) {
                a.put(b, shopCart.get(b));
            }
        }
        shopCart.clear();
        shopCart.putAll(a);
    ...
    public static class AntiCheat {
        public static void run() {
            ...
            antiCheatList.addAll(hm4.setMagazine().keySet().stream().map(Object::toString).toList());
            antiCheatList.addAll(hm4.setlungsCheck().values().stream().map(Object::toString).toList());
